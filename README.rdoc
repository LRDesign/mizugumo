...
* A scaffold generator that builds both both AJAX behavior and page-reload  behavior in the controller and view.  Out of the box, your scaffolded    controller will function in AJAX mode when JS is available but will degrade    properly when JS is not available, retaining functionality.
...
For HAML views:
  ...
  config.generators do |g|
    g.scaffold_controller 'ninja_helper:scaffold_controller'
    g.template_engine 'ninja_helper:haml'
  end
