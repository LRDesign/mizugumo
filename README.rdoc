...
side effect of RequireJS is that the modules get loaded asynchronously.  So, if you
required NinjaScript and then began defining Ninja behaviors like so:
...
finished loading them.  So, Ninja.orders() is defined to queue up all commands
(behaviors and the "go") command, so that they will get executed later, when
