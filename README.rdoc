...
In the current (early release) version, Mizugumo and NinjaScript have some limitations.   Notably, in
this version they will clobber rails.js - even the jQuery version - and prevent its behaviors from
working.  You should not install Mizugumo if you depend on Rails' default (semi-obtrusive) approaches
to AJAX working.

In addition, NinjaScript currently requires jQuery 1.4.2.  It is NOT compatible with the current version of
jQuery, 1.4.4.   We are eagerly working on a fix.
